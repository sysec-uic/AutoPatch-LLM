...
def test_str(monkeypatch):
    ...
    test_instance = MessageBrokerClient("localhost", 1883, logger)
...
def test_connect_message_broker_success(monkeypatch):
    logger = mock.Mock(spec=logging.Logger)
    monkeypatch.setattr(mqtt_client.Client, "connect", mock_connect)
    client = MessageBrokerClient("localhost", 1883, logger)
...
def test_connect_message_broker_failure(monkeypatch):
    logger = mock.Mock(spec=logging.Logger)
    ...
    with pytest.raises(Exception, match="Connection failed"):
        MessageBrokerClient("localhost", 1883, logger)
    logger.info.assert_called_with(
        "MessageBrokerClient - Connecting to MQTT Broker on localhost:1883"
    )
...
def test_on_disconnect_reconnect(monkeypatch):
    logger = mock.Mock(spec=logging.Logger)
    ...
    logger.info.assert_any_call("MessageBrokerClient - Reconnecting in 1 seconds...")
    logger.info.assert_any_call("MessageBrokerClient - Reconnected successfully!")
...
def test_publish_with_empty_topic(monkeypatch):
    """
...
def test_on_disconnect_no_reconnect(monkeypatch):
    """
