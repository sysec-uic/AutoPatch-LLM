...
In the previous chapters, we explored several specialized services within AutoPatch: finding crashes ([Fuzzing Service](01_fuzzing_service_.md)), analyzing code ([CPG Generation Service](02_code_property_graph__cpg__generation_service_.md)), generating patches with AI ([LLM Dispatch & Patch Generation Service](03_llm_dispatch___patch_generation_service_.md)), and testing those patches ([Patch Evaluation Service](04_patch_evaluation_service_.md)).

But how do these different services talk to each other? When the Fuzzing Service finds a crash, how does it tell the Patch Evaluation Service exactly *what* crashed and *how*? When the LLM Service generates a patch, how does it package that patch so the Evaluation Service knows what to test?
