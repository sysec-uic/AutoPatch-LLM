...
INPUT_PATH = os.environ.get("INPUT_PATH", "/app/src/input/")
PATCHED_CODES_PATH = os.environ.get("PATCHED_CODES_PATH", "/app/src/patched_codes/")
...
timeouts = [
    ...
    60,
    120,
]  # Increasing timeouts so that it's more likely to find crashes on complex codes
...
def main():
    ...
    for code in os.listdir(CODEBASE_PATH):
        ...
        for _ in range(max_tries):
            ...
            if (
                res
            ):  # The code is syntactically correct, so an executable file exists for it
            ...
            if (
                not res
            ):  # The code is not syntactically correct, so we have no crashes to use to test it
                for (
                    t
                ) in (
                    timeouts
                ):  # Run the fuzzer for increasing timeouts until some crash is found
